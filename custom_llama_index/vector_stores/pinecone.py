...
from custom_llama_index.vector_stores.types import (
    NodeEmbeddingResult,
    VectorStore,
...
class PineconeVectorStore(VectorStore):
    ...
    def add(
        ...
        for result in embedding_results:
            ...
            metadata.update(result.doc_metadata)
            self._pinecone_index.upsert(
                [(new_id, text_embedding, metadata)], **self._pinecone_kwargs
            )
            fileObj=ProjectData.objects.get(data_id=result.doc_id)
            if fileObj.extra_data:
                chunk_data = json.loads(fileObj.extra_data)
                chunk_list = chunk_data.get("chunk_ids","")
                if not chunk_list:
                    chunk_list = []
            else:
                chunk_list = []

            chunk_list.append(new_id)
            fileObj.extra_data = json.dumps({"chunk_ids": chunk_list})
            fileObj.save()
            ids.append(new_id)
        return ids
