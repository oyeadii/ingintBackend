...
from custom_llama_index.readers.weaviate.utils import (
    get_by_id,
    parse_get_response,
    validate_client,
)
...
class BaseWeaviateIndexStruct(Generic[IS]):
    ...
    @classmethod
    @abstractmethod
    def _entry_to_custom_llama_index(cls, entry: Dict) -> IS:
        """Convert to LlamaIndex list."""

    @classmethod
    def to_custom_llama_index_list(
        cls,
        client: Any,
        class_prefix: str,
        ...
        for entry in entries:
            results.append(cls._entry_to_custom_llama_index(entry))

        return results

    @classmethod
    @abstractmethod
    def _from_custom_llama_index(
        cls, client: Any, index: IS, class_prefix: str, batch: Optional[Any] = None
    ) -> str:
        """Convert from LlamaIndex."""

    @classmethod
    def from_custom_llama_index(cls, client: Any, index: IS, class_prefix: str) -> str:
        """Convert from LlamaIndex."""
        validate_client(client)
        index_id = cls._from_custom_llama_index(client, index, class_prefix)
        client.batch.flush()
        return index_id
...
class WeaviateNode(BaseWeaviateIndexStruct[Node]):
    ...
    @classmethod
    def _entry_to_custom_llama_index(cls, entry: Dict) -> Node:
        """Convert to LlamaIndex list."""
    ...
    @classmethod
    def from_custom_llama_index_batch(
        ...
        with client.batch as batch:
            for node in nodes:
                index_id = cls._from_custom_llama_index(client, node, class_prefix, batch=batch)
