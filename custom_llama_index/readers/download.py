...
def download_loader(
    ...
    use_custom_llama_index_import: bool = False,
) -> Type[BaseReader]:
    ...
    Args:
        ...
        use_custom_llama_index_import: If true, the loader files will use
            custom_llama_index as the base dependency. By default (False),
            the loader files use custom_llama_index as the base dependency.
            NOTE: this is a temporary workaround while we fully migrate all usages
            to custom_llama_index.
    ...
    if refresh_cache or not os.path.exists(loader_path):
        ...
        if use_custom_llama_index_import:
            basepy_raw_content = basepy_raw_content.replace(
                "import custom_llama_index", "import custom_llama_index"
            )
            basepy_raw_content = basepy_raw_content.replace(
                "from custom_llama_index", "from custom_llama_index"
            )
