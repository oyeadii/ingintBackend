...
from custom_llama_index.async_utils import run_async_tasks
from custom_llama_index.data_structs.data_structs import IndexDict, Node
from custom_llama_index.embeddings.base import BaseEmbedding
from custom_llama_index.indices.base import DOCUMENTS_INPUT, BaseGPTIndex
from custom_llama_index.indices.query.base import BaseGPTIndexQuery
from custom_llama_index.indices.query.schema import QueryMode
from custom_llama_index.indices.query.vector_store.base import GPTVectorStoreIndexQuery
from custom_llama_index.langchain_helpers.chain_wrapper import LLMPredictor
from custom_llama_index.langchain_helpers.text_splitter import TextSplitter
from custom_llama_index.prompts.default_prompts import DEFAULT_TEXT_QA_PROMPT
from custom_llama_index.prompts.prompts import QuestionAnswerPrompt
from custom_llama_index.schema import BaseDocument
from custom_llama_index.utils import get_new_id
from custom_llama_index.vector_stores.simple import SimpleVectorStore
from custom_llama_index.vector_stores.types import NodeEmbeddingResult, VectorStore
...
class GPTVectorStoreIndex(BaseGPTIndex[IndexDict]):
    ...
    def _get_node_embedding_results(
        self, nodes: List[Node], existing_node_ids: Set, doc_id: str, doc_metadata: dict, 
        ...
        for id, embed in id_to_embed_map.items():
            result_tups.append(
                NodeEmbeddingResult(id, id_to_node_map[id], embed, doc_id=doc_id, doc_metadata=doc_metadata)
            )
        return result_tups
    ...
    async def _async_add_document_to_index(
        ...
        embedding_results = await self._aget_node_embedding_results(
            nodes, set(), document.get_doc_id(), document.get_doc_metadata()
        )
